...
for pc_path in glob.glob(join(dataset_path, '*.txt')):
    ...
    if exists(join(sub_pc_folder, f'{file_name}_KDTree.pkl')):
        continue

    pc = DP.load_pc_semantic3d(pc_path)
    # check if label exists
    label_path = f'{pc_path[:-4]}.labels'
    if exists(label_path):
        labels = DP.load_label_semantic3d(label_path)
        full_ply_path = join(original_pc_folder, f'{file_name}.ply')
        ...
        sub_labels = np.squeeze(sub_labels)
        sub_ply_file = join(sub_pc_folder, f'{file_name}.ply')
        write_ply(sub_ply_file, [sub_xyz, sub_colors, sub_labels], ['x', 'y', 'z', 'red', 'green', 'blue', 'class'])

        search_tree = KDTree(sub_xyz, leaf_size=50)
        kd_tree_file = join(sub_pc_folder, f'{file_name}_KDTree.pkl')
    ...
    proj_save = join(sub_pc_folder, f'{file_name}_proj.pkl')
